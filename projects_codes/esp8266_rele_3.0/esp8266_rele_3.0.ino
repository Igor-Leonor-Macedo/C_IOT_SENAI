 ...
 *
 *  VERSÃO COM SINCRONIZAÇÃO MULTI-DISPOSITIVO:
 *  Todos os navegadores conectados consultam /status
 *  a cada 1,5 s e atualizam os botões automaticamente.
 * =====================================================
 *
 ...
 *  4. Acesse o IP no navegador de vários dispositivos
 * =====================================================
 */
...
// ─────────────────────────────────────────────
const char* ssid     = "TP-Link_425A";
const char* password = "=Ev$fL+7";
...
<body>
  ...
  <footer>Estado sincronizado entre todos os dispositivos conectados</footer>

  <script>
    // Intervalo de sincronização (ms). Todos os navegadores
    // consultam /status periodicamente e atualizam os cards.
    const INTERVALO_SYNC = 1500;
    ...
    // Busca o estado real dos 3 relés no ESP8266 e
    // atualiza a interface. Assim, quando outro
    // dispositivo aciona um relé, este navegador
    // reflete a mudança em até 1,5 s.
    async function sincronizarEstados() {
      try {
        const resp = await fetch('/status');
        const data = await resp.json();
        data.estados.forEach((ligado, id) => {
          const btn = document.getElementById('btn' + id);
          // não sobrescreve um card cujo botão acabou
          // de ser clicado neste dispositivo
          if (btn && !btn.disabled) atualizarCard(id, ligado);
        });
      } catch(e) {
    ...
    setInterval(sincronizarEstados, INTERVALO_SYNC);
  </script>
...
// ─── NOVO: retorna o estado atual dos 3 relés ──
// Todos os dispositivos consultam esta rota
// periodicamente para se manterem sincronizados.
// Resposta: {"estados":[true,false,false]}
void handleStatus() {
  String json = "{\"estados\":[";
  for (int i = 0; i < 3; i++) {
    json += estadoRele[i] ? "true" : "false";
    if (i < 2) json += ",";
  }
  json += "]}";
  server.send(200, "application/json", json);
...
void setup() {
  Serial.begin(115200);
  Serial.println("\n\n=== ESP8266 — Painel 3 Relés (sincronizado) ===");
  ...
  server.on("/status",   handleStatus);   // NOVO: rota de sincronização
  server.begin();
